...
def module_list_to_verilog_module(
    ...
    module_contents = ""
    for i in tqdm(range(len(module_list)), desc="Generating Verilog", unit="layer"):
        m = module_list[i]
...
class SparseLinear(nn.Linear):
    def __init__(
        self, in_features: int, out_features: int, mask: nn.Module, bias: bool = True
    ) -> None:
...
class SparseLinearNeq(nn.Module):
    ...
    def gen_layer_verilog(self, module_prefix, directory, generate_bench: bool = True):
        ...
        for index in tqdm(
            ...
            desc="Generating verilog for layer",
            unit="feature",
        ):
    ...
    def calculate_truth_tables(self):
        with torch.no_grad():
            ...
            for n in range(self.out_features):
                ...
                self.output_quant.float_output()
                output_states = self.output_quant(self.forward(padded_perm_matrix))[
                    :, n
                ]  # Calculate float for the current input
                self.output_quant.bin_output()
                bin_output_states = self.output_quant(self.forward(padded_perm_matrix))[
                    :, n
